...
for _ in range(nlayers):
    model.add(LSTM(ncells, input_shape=(lookback, n+1), return_sequences=True, activation='tanh'))
model.add(LSTM(ncells, input_shape=(lookback, n+1), activation='tanh'))
#model.add(Dense(40, activation='tanh'))
model.add(Dense(n))

print(model.summary())
tf.keras.utils.plot_model(model, to_file=f'ml_rom_{ReTest}_{Re[pref]}.png', show_shapes=True)
...
np.savez(f'./plotting/modal_coefficients_{ReTest}_{Re[pref]}.npz', t = t, aTest = aTest, aGPtest = aGPtest, aML = aML_all_seeds)

np.savez(f'./plotting/vorticity_field_{ReTest}_{Re[pref]}.npz', w_fom = w_fom, w_test = w_test, w_gp = w_gp,
         w_ml = w_ml)
